...
function MealsPlan() {
        ...
        cell: (row) => {
          return (
            <div className="gap-2">
              ...
              <button
                onClick={async () =>
                  await changeAvailabilityMealPlanCW(row._id)
                    .then(fetchAllMeals)
                    .then(() => toast.success(`${row.title} plan updated`))
                }
              >
                {row.active ? (
                  <img className="image-button h-7" src={tickIcon} alt="active meal" />
                ) : (
                  <img className="image-button h-7" src={disableIcon} alt="disabled meal" />
                )}
              </button>
            </div>
          );
  ...
  return (
    <div className="parent-container ">
      ...
      <Modal isOpen={modalOpen} heading="Meal Plan Details" closeHandler={setModal}>
        {modalData && (
          <MealPlanForm
